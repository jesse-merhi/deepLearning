...
def main():
    ...
    if args.net == "lin":
        ...
        print("Total Parameters: ", total_params)
        number = (
            4 * 4 * 32  # The Picture size times Convolution
            + 32  # Biases
            + 4 * 4 * 32 * 64  # Convolution times Bigger Convolution
            + 64  # Biases
            + 64 * 22 * 22 * 128  # Big Conv times linear
            + 128  # Biases
            + 128 * 10  # Linear Function to output
            + 10  # Biases
        )
        print("My calc", number)
        print("Diff", number - total_params)
        exit(1)
