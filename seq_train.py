...
parser.add_argument(
    "--init", type=float, default=0.001, help="initial weight size (srn)"
)
# training options
parser.add_argument(
    "--epoch", type=int, default=0, help="number of training epochs ('000s)"
)
...
if args.optim == "adam":
    ...
    optimizer = optim.SGD(
        net.parameters(), lr=args.lr, momentum=args.mom, weight_decay=0.00001
    )
    ...
    if epoch % 1000 == 0:
        ...
        if epoch % 10000 == 0:
            ...
            torch.save(
                net.state_dict(),
                path
                + "%s_%s%d_%d.pth" % (args.lang, args.model, args.hid, epoch / 1000),
            )
